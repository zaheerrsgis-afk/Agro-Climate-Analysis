...
.panel { background: #ffffff; border: 1px solid #e7eaf0; border-radius: 18px; box-shadow: 0 4px 16px rgba(15, 23, 42, 0.06); padding: 14px; margin-bottom: 14px; }
.step-title { display: flex; align-items: center; gap: 8px; color: #166534; font-weight: 800; font-size: 1.05rem; margin-bottom: 12px; }
.step-badge { width: 22px; height: 22px; background: linear-gradient(135deg, #22c55e, #86efac); color: white; border-radius: 999px; display: inline-flex; align-items: center; justify-content: center; font-size: 0.82rem; font-weight: 900; }
.tab-row { display: flex; gap: 18px; border-bottom: 1px solid #e5e7eb; margin-bottom: 12px; }
.tab-active { color: #15803d; border-bottom: 3px solid #22c55e; padding-bottom: 9px; font-weight: 800; }
.tab-muted { color: #6b7280; padding-bottom: 9px; font-weight: 600; }
.small-help { color: #9ca3af; font-size: 0.82rem; margin-top: 4px; }
.module-card { border: 1px solid #86efac; border-radius: 12px; padding: 12px; margin: 0; background: #f0fdf4; }
.module-title { font-weight: 800; color: #1f2937; }
.module-sub { color: #9ca3af; font-size: 0.82rem; }
.map-card { background: #ffffff; border: 1px solid #86efac; border-radius: 18px; box-shadow: 0 8px 24px rgba(34,197,94,0.12); padding: 10px; }
.map-header { position: relative; z-index: 2; margin-bottom: 8px; display: flex; gap: 8px; align-items: center; flex-wrap: wrap; }
.pill { background: rgba(255,255,255,0.92); border: 1px solid #e5e7eb; border-radius: 999px; padding: 5px 10px; font-size: 0.82rem; color: #374151; font-weight: 700; }
.result-title { color: #166534; font-weight: 900; font-size: 1.05rem; margin-bottom: 10px; }
.alert-box { background: #fef3c7; border: 1px solid #f59e0b; border-radius: 12px; padding: 12px; color: #92400e; font-size: 0.92rem; }
...
def download_rasters_from_drive():
    RASTER_DIR.mkdir(parents=True, exist_ok=True)
    if not missing_required_rasters():
        return True, "Rasters already available."
    try:
        ...
        return True, "Rasters downloaded successfully."
    except Exception as e:
        return False, f"Google Drive raster download failed: {e}"


def ensure_rasters_available():
    if not missing_required_rasters():
        return True
    with st.spinner("Downloading raster layers from Google Drive. First load may take a few minutes..."):
        ok, msg = download_rasters_from_drive()
    if not ok:
        st.error(msg)
        st.info("Check that the Google Drive folder is public and contains all required TIFF files. Folder sharing should be: Anyone with the link → Viewer.")
        return False
    return True
...
def clean_geojson(gj):
    ...
    for ft in gj.get("features", []):
        ...
        if not isinstance(geom, dict):
            continue
        if geom.get("type") in ["Polygon", "MultiPolygon", "LineString", "MultiLineString", "Point", "MultiPoint"] and geom.get("coordinates"):
...
def load_boundary_geojson(geojson_path: str, shp_path: str):
    ...
    if file_available(shp):
        try:
            ...
            st.error("Shapefile found, but geopandas is not installed. Use GeoJSON or install geopandas.")
            return None, "Missing geopandas"
...
def raster_to_overlay_payload(path, layer_name, shapes_json, use_balanced_classes, max_size=650):
    ...
    with rasterio.open(p) as src:
        ...
        if src_crs and src_crs.to_string() != "EPSG:4326":
            dst_transform, dst_width, dst_height = calculate_default_transform(src_crs, "EPSG:4326", src.width, src.height, *src.bounds)
        ...
        if use_balanced_classes:
            arr = classify_balanced(data)
        else:
            arr = np.rint(data).astype(np.int16)
            arr[~np.isfinite(data)] = 0
        rgba = class_to_rgba(arr, LAYER_CONFIG[layer_name]["legend"], alpha=175)
        buf = io.BytesIO()
        plt.imsave(buf, rgba)
        buf.seek(0)
        encoded = base64.b64encode(buf.getvalue()).decode("utf-8")
        west, south, east, north = rasterio.transform.array_bounds(data.shape[0], data.shape[1], transform)
        return {"image": f"data:image/png;base64,{encoded}", "bounds": [[south, west], [north, east]]}


def render_map(selected_layer, selected_district, use_balanced_classes, opacity, boundary_gj):
    ...
    m = folium.Map(location=[31.2, 72.9], zoom_start=7, tiles="CartoDB positron", control_scale=True)
    folium.TileLayer("OpenStreetMap", name="OpenStreetMap", control=True).add_to(m)
    try:
        overlay = raster_to_overlay_payload(str(raster_path), selected_layer, shapes, use_balanced_classes)
        if overlay:
    ...
    if selected_feature:
        ...
        folium.GeoJson(
            ...
            style_function=lambda feature: {"fillColor": "transparent", "color": "#16a34a", "weight": 2.5, "fillOpacity": 0.0, "opacity": 1.0},
            tooltip=tooltip,
        ).add_to(m)
...
def render_legend(layer_name):
    ...
    for _, (label, color) in cfg["legend"].items():
        rows += f'<div class="legend-row"><div class="legend-swatch" style="background:{color};"></div><span>{label}</span></div>'
...
if district_col:
    ...
    prop = find_district_property(boundary_gj)
    if prop:
        district_names += sorted([str(f.get("properties", {}).get(prop)) for f in boundary_gj.get("features", []) if f.get("properties", {}).get(prop) is not None])
...
with left_col:
    ...
    use_balanced_classes = st.checkbox("Balanced visual classes", value=False)
    opacity = st.slider("Layer opacity", 0.10, 1.00, 0.62, 0.05)
    st.markdown('</div>', unsafe_allow_html=True)

layer_stats = dynamic_layer_stats(scorecard, selected_layer, selected_district, use_balanced=use_balanced_classes)
...
with right_col:
    ...
    if not scorecard.empty and district_col and selected_district != "All Punjab":
        ...
        st.download_button("Download District Profile", data=row.to_csv(index=False).encode("utf-8"), file_name=f"{selected_district.replace(' ', '_')}_Risk_Profile.csv", mime="text/csv", use_container_width=True)
    ...
    if boundary_gj:
        st.download_button("Download AOI GeoJSON", data=json.dumps(boundary_gj).encode("utf-8"), file_name="Punjab_Districts.geojson", mime="application/geo+json", use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)
...
with tab1:
    ...
    if scorecard.empty:
        ...
        work["Selected_Layer_Risk"] = layer_risk_labels(work, selected_layer, use_balanced=use_balanced_classes)
...
with tab3:
    ...
    st.markdown(
        ...
        The map is clipped to Punjab boundaries, and district selection clips the result raster to the selected district.
        Rasters are stored on Google Drive and downloaded automatically at first load to keep the GitHub repository light.
        ...
        **Performance note:** For Streamlit Cloud, public display rasters should be exported at approximately 10 km scale and kept as small classified GeoTIFFs.
        """
    )
