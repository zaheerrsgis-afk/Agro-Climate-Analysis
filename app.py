...
#   data/vectors/Punjab_Districts.shx
#   data/vectors/Punjab_Districts.dbf
#   data/vectors/Punjab_Districts.prj
#
# If shapefile fallback is used, install:
...
# =====================================================
# Google Drive Raster Folder
# =====================================================
# Rasters are stored on Google Drive to keep GitHub repository light.
...
OPTIONAL_SCORE_RASTER_FILES = [
    "Punjab_Environmental_Stress_Score.tif",
    "Punjab_Drought_Prone_Score.tif",
    "Punjab_Water_Storage_Stress_Score.tif",
]
...
LAYER_CONFIG = {
    "Environmental Stress Hotspots": {
        ...
        "score_file": "Punjab_Environmental_Stress_Score.tif",
        "score_col": "Environmental_Stress_Score",
        "risk_col": "Overall_Risk",
        "description": "Overall long-term environmental stress combining water, drought, crop, heat and soil moisture indicators.",
    ...
    "Drought-Prone Areas": {
        ...
        "score_file": "Punjab_Drought_Prone_Score.tif",
    ...
    "Water Storage Stress": {
        ...
        "score_file": "Punjab_Water_Storage_Stress_Score.tif",
...
def missing_required_rasters():
    """Return public raster files that are missing locally."""
...
def download_rasters_from_drive():
    """
    Download raster files from a public Google Drive folder.
    This is cached by Streamlit, so it should run only once per app session.
    """
    RASTER_DIR.mkdir(parents=True, exist_ok=True)
...
def ensure_rasters_available():
    """Download rasters from Google Drive if data/rasters is empty or incomplete."""
    if not missing_required_rasters():
        return True
    ...
    if not ok:
        st.error(msg)
        st.info(
            "Please make sure the Google Drive raster folder is public: "
            "Share → Anyone with the link → Viewer. "
            "Also confirm TIFF file names match the expected names."
        )
        return False
...
def load_boundary_geojson(geojson_path: str, shp_path: str):
    """
    Load boundary from GeoJSON first.
    If GeoJSON does not exist, fallback to shapefile using geopandas.
    """
    ...
    if file_available(shp):
        try:
            ...
            st.error(
                "Shapefile found, but geopandas is not installed. "
                "Run: python -m pip install geopandas pyogrio shapely"
            )
            return None, "Missing geopandas"

        gdf = gpd.read_file(shp)
        if gdf.crs is None:
            st.warning("Shapefile CRS is missing. Assuming EPSG:4326.")
            gdf = gdf.set_crs("EPSG:4326")
        else:
            gdf = gdf.to_crs("EPSG:4326")

        gj = json.loads(gdf.to_json())
        return clean_geojson(gj), "Shapefile"
...
def selected_or_province_shapes(gj, district_name):
    ...
    feats = province_features(gj)
    if feats:
        return [f["geometry"] for f in feats if f.get("geometry")], None
...
def class_to_rgba(arr, legend, alpha=185):
...
@st.cache_data(show_spinner=False)
def raster_to_overlay_payload(path, layer_name, shapes_json, use_balanced_classes, max_size=1300):
    p = Path(path)
    ...
    with rasterio.open(p) as src:
        ...
        rgba = class_to_rgba(arr, LAYER_CONFIG[layer_name]["legend"], alpha=185)
...
# Download rasters from Google Drive if missing
...
with left_col:
    ...
    st.markdown(f'<div class="small-help">Boundary source: {boundary_source}. Rasters load from Google Drive. Choose All Punjab or zoom to one district.</div>', unsafe_allow_html=True)
    ...
    use_balanced_classes = st.checkbox("Balanced visual classes", value=True)
    opacity = st.slider("Layer opacity", 0.10, 1.00, 0.68, 0.05)
    st.markdown('</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    st.markdown(
        """
        This dashboard uses backend satellite indicators from 2018–2024 to prepare public-facing risk layers for Punjab.
        ...
        **Boundary input:** The dashboard reads `Punjab_Districts.geojson` first. If it is not available, it falls back to `Punjab_Districts.shp`.
