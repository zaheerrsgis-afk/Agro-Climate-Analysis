...
def choose_raster_path(layer_name):
    """Use local public 1–4 class rasters only.

    Upload these GeoTIFFs directly in GitHub under data/rasters/.
    """
    cfg = LAYER_CONFIG[layer_name]
...
with left_col:
    ...
    st.markdown(f'<div class="small-help">Boundary source: {boundary_source}. Choose All Punjab or zoom to one district.</div>', unsafe_allow_html=True)
...
with map_col:
    ...
    if not (RASTER_DIR / raster_name).exists():
        st.error(f"Local raster is missing: data/rasters/{raster_name}")
